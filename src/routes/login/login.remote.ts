...
import { BadRequest, Redirect } from "$lib/server/remote/responses";
import { verify } from "@node-rs/argon2";
import { eq } from "drizzle-orm";
import { Effect, Redacted } from "effect";

export const login = form(loginSchema, ({ email, _password: password }) =>
  Effect.gen(function* () {
    const db = yield* ORM;
    ...
    const session = yield* auth.createSession(sessionToken, existingUser.id);
    yield* auth.setSessionTokenCookie(event, sessionToken, session.expiresAt);

    return new Redirect({ to: resolve("/demo/lucia"), code: 302 });
